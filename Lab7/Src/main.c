	  ...
	  if(drdyFlag == 1)
	  	  {
	  		  ...
	  		  const float tiltThreshold = 100.0f;

	  		  // Detect forward tilt (blue LED)
	  		  if (myData.y < -tiltThreshold) {
	  		  ...
	  		  // Detect backward tilt (orange LED)
	  		  else if (myData.y > tiltThreshold) {
	  			       HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_RESET);
	          ...
	          // Detect left tilt (green LED)
	  		  else if (myData.x < -tiltThreshold) {
